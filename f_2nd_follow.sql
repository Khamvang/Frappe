...
SELECT sme.id `#`, 
	...
	CASE 
		WHEN tftl.date_created >= CURDATE() OR tfse.date_created >= CURDATE() OR tfcr.date_created >= CURDATE() THEN 'Called'
		WHEN bp.modified > CURDATE() THEN 'Called'
		ELSE 'x'
	END AS `is_called`,
	CASE 
...
SELECT * FROM temp_sme_id_report WHERE `type` = 'Next week plan' AND datetime_report = MAX(datetime_report)
